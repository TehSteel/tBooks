...
public class RoleAuthorizationFilter extends AbstractGatewayFilterFactory<RoleAuthorizationFilter.Config> {
	...
	@Override
	public GatewayFilter apply(final RoleAuthorizationFilter.Config config) {
		return ((exchange, chain) -> {
			...
			if (JwtUtil.getClaims(token).get("role", UserRole.class) != UserRole.ADMIN) {
				return onError(exchange, HttpStatus.UNAUTHORIZED);
			}
	...
	public static class Config {
		...
		public void setAllowedRoles(final Set<UserRole> allowedRoles) {
